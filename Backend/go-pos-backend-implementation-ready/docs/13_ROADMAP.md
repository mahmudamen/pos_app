...
- [x] B3 Multi-device sync (pull side): `GET /v1/sync/pull?cursor=&limit=` streams tenant changes ordered by the monotonic `change_seq` (migration `014_sync_pull` adds `change_seq` to `register_sessions`, `inventory_adjustments`, `tenant_settings`). Entities: categories, products, customers, sales, customer_loyalty_log, register_sessions, inventory_adjustments, tenant_settings — soft-deleted catalog rows decode as `change_type: delete`. Bounded pages (`limit` ≤ 500), `has_more`/next-`cursor` for incremental replay, and `cursor_expired` (409) when a cursor predates the retained window. Users/devices/sync_commands excluded (sensitive). Push side stays backlog: offline `create_sale` queue on the client is the first consumer; server-side push endpoint not yet planned (see decision log).
