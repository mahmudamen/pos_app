...
func clearEnv() {
	// Every key config.Load reads, so the "defaults" tests are hermetic even
	// when the test runner was started from a shell with .env exported.
	keys := []string{
		...
		"CASHIER_DISCOUNT_PCT", "MAX_SESSIONS_PER_USER",
		"LOGIN_RATE_MAX", "LOGIN_RATE_WINDOW",
		...
		"MAX_ACTIVE_INSTALLATIONS", "REGISTER_RATE_PER_IP_PER_HOUR",
