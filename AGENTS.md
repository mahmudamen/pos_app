...
- **OpenAPI generation (E5)**: `cmd/openapi` builds the engine via `server.Register` with a nil pool + bare config (runs offline, no DB), walks the live Gin route table, and emits `docs/openapi.json` (40 paths; `{id}` params; bearerAuth security except `/v1/auth/login` + `/v1/meta/*` + `/health/*`; shared data/error `Envelope`). `make openapi` regenerates it.
...
### Backend — tests (308 test functions; 286 pass + 22 skip offline, `go vet` clean)
...
| `sales` handler | 33 | 25 unit (list/get/create, auth required, unavailable, pagination, validation, writeSale/writeError, normalizePayments/primaryPaymentMethod (split tender) + tip aggregation/rejection + `validateDiscount` role caps + `saleCustomerID`, `loyaltyRateFromValue`, `pointsForTotal`) + 8 skips offline: 6 validation-order (503-before-400 asserts when the pool is nil) + 2 DB-backed integration (SALE-008: idempotent duplicate + 5-way concurrent → exactly 1 sale row, stock once) |
...
| `receipts` handler | 13 | ESC/POS byte-level layout (escp_test) + JSON/print handler routing, 404/400, 503 without DB |
...
### Flutter — tests (111 passing, `flutter analyze` 0 issues)
| File | Tests | Coverage |
|------|-------|----------|
...
- **OpenAPI spec**: auto-generate from Gin routes, publish for frontend codegen (E5 — shipped: `make openapi` → `docs/openapi.json` 40 paths; schemas for write bodies are hand-augmented as the contract grows)