...
### Backend — tests (344 test functions; the full suite runs against a real Postgres as non-superuser `pos_test_user` — 344 PASS / 6 SKIP offline — `go vet` + `go test -race` clean)

| Package | Tests | Coverage |
|---------|-------|----------|
| `config` | 24 | env parsing, defaults, durations, overrides, `CASHIER_DISCOUNT_PCT` bounds/parse, `CORS_ALLOWED_ORIGINS` parse + wildcard default, `METRICS_ENABLED` bool parse + rejection |
...
| `auth` handler | 23 | 19 unit (login/refresh/logout happy paths, invalid JSON, missing fields, bad email, unavailable, route registration, PIN set/verify dispatch) + 4 DB-backed integration (AUTH-011: login happy path/wrong-password 401, refresh rotation + reuse revocation, logout session revocation) |
...
| `sales` handler | 43 | 39 unit (list/get/create, auth required, unavailable, pagination, validation, writeSale/writeError, normalizePayments/primaryPaymentMethod (split tender) + tip aggregation/rejection + `validateDiscount` role caps + `saleCustomerID`, `loyaltyRateFromValue`, `pointsForTotal` + refund validation/lines math) + 4 DB-backed integration (SALE-008: idempotent duplicate + 5-way concurrent → exactly 1 sale row, stock once + REFUND: restock round-trip, staff echoes) |
...
- **On-device E2E (E4)**: `integration_test/network_e2e_test.dart` runs the full login → countries/currencies → catalog → register-open → sale → receipt → logout flow against a real backend (`flutter test integration_test/network_e2e_test.dart -d <dev> --dart-define=API_BASE_URL=http://<host>:<port>`); the host-runnable offline-replay widget test covers the same sale path in CI (126 host tests).

### Flutter — tests (150 passing, `flutter analyze` 0 issues)
| File | Tests | Coverage |
|------|-------|----------|
...
- [x] **Manager PIN (B1)**: set/verify/remove PIN endpoints (`021_users_pin`, bcrypt) landed so refunds/discount-overrides can be manager-approved on-device; hardware PIN dialog is a Flutter UI backlog item.

### Backlog (kept from prior plan; see roadmap for details & why items are skipped)
- ~~**Manager PIN for refunds/discounts**~~ shipped server-side (B1/C5): set/verify/remove PIN + `pos.refund`; remaining is the on-device manager-PIN modal UI in Flutter
- **Cashier security / RBAC extension**: refunds (`pos.refund`) + PIN shipped; remaining: per-tenant permission overrides
