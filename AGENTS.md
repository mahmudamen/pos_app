...
- Migrations are goose files in `internal/infrastructure/database/migrations/` (single-file `.sql`, numbered `001_extensions`, `003_core`, `004_permissions`, `005_sync`, `006_tenant_type`, `007_localization`, `008_payments`, `010_settings`, `011_registers`, `012_inventory_adjustments`, `013_loyalty`, `014_sync_pull`); apply with `make migrate`, roll back `make migrate-down`.
...
### Backend — tests (230 total, `go vet` clean)
...
| `database` | 2 | nil pool, close without connect (1 skip: RLS integration) |
...
| `catalog` handler | 49 | CRUD, pagination, search, barcode, validation, PATCH, category PATCH/DELETE, product soft-delete, errors (5 skips: integration DB tests) |
...
- **Multi-device sync (B3, pull side)**: migration `014_sync_pull` adds `change_seq` + `bump_change_seq` triggers to `register_sessions`, `inventory_adjustments`, `tenant_settings` (append-only column for adjustments). `GET /v1/sync/pull?cursor=&limit=` streams tenant changes (categories, products, customers, sales, customer_loyalty_log, register_sessions, inventory_adjustments, tenant_settings) ordered by monotonic `change_seq`, bounded pages with `has_more` + next-`cursor`, soft-deleted catalog rows as `change_type: delete`, and `cursor_expired` (409) for cursors older than the retained window; users/devices excluded. Module `internal/transport/sync/`. Flutter: `ApiClient.syncPull` + `SyncRow`/`SyncPullPage` models, `SessionStore.readSyncCursor`/`saveSyncCursor` (per-tenant secure-storage key `sync_cursor_<tenant>`).

### Flutter — tests (104 passing, `flutter analyze` 0 issues)
| File | Tests | Coverage |
|------|-------|----------|
| `api_client_test.dart` | 53 | login, logout, products, categories, sales, refresh, createProduct, updateProduct, session, Product.fromJson, _message edge cases, Country/Currency parsing + meta fetch, SaasSummary/SaasTenant parsing, split-payment request, SaleResult/SaleSummary payment method, register session models current/open/close/sessions/session_id-on-createSale, DashboardSummary parse + fetch + error, Customer parse + listCustomers + createCustomer, syncPull fetch with cursor+limit, syncPull cursor_expired → ApiException, SyncRow parse (delete row) |
...
- **Multi-device sync**: pull side shipped (B3 — `GET /v1/sync/pull` via `change_seq`); remaining: push endpoint (`POST /v1/sync/push`), offline→online replay via sync, conflict resolution (idempotency already in place)
